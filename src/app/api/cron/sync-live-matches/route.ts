...
export async function PUT(request: Request) {
	try {
		...
		for (const doc of matchDocs) {
			...
			const espnMatchId: string | undefined = stored.espnMatchId;
			try {
				const apiMatch = espnMatchId
				...
				const scoreChanged =
					newHomeScore !== stored.scoreHome ||
					newAwayScore !== stored.scoreAway;
				...
				const penaltiesChanged =
					(apiMatch.score?.penalties?.home ?? null) !==
						stored.scorePenaltiesHome ||
					(apiMatch.score?.penalties?.away ?? null) !==
						stored.scorePenaltiesAway;
