...
export default function LeaderboardView({ currentUser, users }: LeaderboardViewProps) {
    ...
    <div className="flex-1 bg-slate-950 text-slate-100 min-h-screen pb-16">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-slate-900/80 border-b border-slate-800 px-4 py-4 sm:px-8">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            <h2 className="text-lg font-extrabold text-white">Tabla de Posiciones</h2>
          ...
          <div className="bg-slate-800/50 border border-slate-700/50 py-1.5 px-3 rounded-full text-xs font-semibold">
            Tu Puesto: <span className="text-amber-400 font-bold">#{currentUser.rank ?? '-'}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 sm:px-8 space-y-10">
        ...
        <div className="relative pt-12 pb-6 px-4 rounded-3xl bg-gradient-to-b from-indigo-950/20 to-slate-900/40 border border-slate-850 flex items-end justify-center gap-2 sm:gap-6 min-h-[300px] overflow-hidden">
          ...
          {top2 && (
            <div className="flex-1 flex flex-col items-center max-w-[130px] z-10">
              ...
              <span className="text-xs font-bold text-slate-200 truncate w-full text-center">
                {top2.displayName === currentUser.displayName ? 'Tú (Leo Messi)' : top2.displayName}
              </span>
              <span className="text-[11px] font-black text-slate-400 mt-0.5">{top2.totalPoints} pts</span>
              ...
              <div className="w-full bg-slate-900/80 border-t border-x border-slate-800 h-16 rounded-t-xl mt-3 flex items-center justify-center shadow-inner">
                <span className="text-[10px] uppercase font-black tracking-widest text-slate-400">Plata</span>
              </div>
            </div>
          )}
          ...
          {top1 && (
            <div className="flex-1 flex flex-col items-center max-w-[150px] z-20">
              {/* Crown Icon */}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-7 h-7 text-amber-400 mb-2 animate-bounce">
              ...
              <span className="text-sm font-black text-white truncate w-full text-center">
                {top1.displayName === currentUser.displayName ? 'Tú (Leo Messi)' : top1.displayName}
              </span>
              <span className="text-xs font-black text-amber-400 mt-0.5">{top1.totalPoints} pts</span>
              ...
              <div className="w-full bg-slate-900 border-t border-x border-amber-500/20 h-24 rounded-t-2xl mt-3 flex items-center justify-center shadow-lg">
                <span className="text-xs uppercase font-black tracking-widest text-amber-400">Oro</span>
              </div>
            </div>
          )}
          ...
          {top3 && (
            <div className="flex-1 flex flex-col items-center max-w-[130px] z-10">
              ...
              <span className="text-xs font-bold text-slate-200 truncate w-full text-center">
                {top3.displayName === currentUser.displayName ? 'Tú (Leo Messi)' : top3.displayName}
              </span>
              <span className="text-[11px] font-black text-slate-400 mt-0.5">{top3.totalPoints} pts</span>
              ...
              <div className="w-full bg-slate-900/80 border-t border-x border-slate-800 h-12 rounded-t-xl mt-3 flex items-center justify-center shadow-inner">
                <span className="text-[10px] uppercase font-black tracking-widest text-amber-700">Bronce</span>
              </div>
            </div>
          )}
        ...
        <div className="overflow-hidden rounded-2xl bg-slate-900/40 border border-slate-900">
          <div className="p-4 border-b border-slate-900 bg-slate-900/20">
            <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider">Resto de Participantes</h3>
          </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-sm">
                      <span className="block font-medium">
                        {item.displayName} {isCurrentUser && <span className="text-[10px] text-amber-400 bg-amber-500/15 px-2 py-0.5 rounded-full border border-amber-500/20 ml-1.5 uppercase font-bold">Tú</span>}
                      </span>
