...
export default function MatchDetailView({
  ...
  const [homeScore, setHomeScore] = useState(prediction?.predictHome.toString() ?? '');
  const [awayScore, setAwayScore] = useState(prediction?.predictAway.toString() ?? '');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await onSavePrediction(match.matchId, parseInt(homeScore), parseInt(awayScore));
      setIsSaving(false);
      setShowSuccess(true);
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 sm:px-8 space-y-6">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-indigo-950/60 to-slate-900/90 border border-slate-800 p-8 shadow-2xl flex flex-col items-center">
          ...
          <div className="text-center mb-6">
            <span className={`inline-block px-4 py-1.5 rounded-full text-xs font-black uppercase border ${
              isLocked 
          ...
          <div className="w-full flex items-center justify-between gap-6 py-6 max-w-xl">
            ...
            <div className="flex-1 flex flex-col items-center text-center gap-3">
              ...
              <span className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">Local</span>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center text-center gap-3">
              ...
              <span className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">Visitante</span>
            </div>
            ...
            <form onSubmit={handleSave} className="max-w-md mx-auto space-y-6 flex flex-col items-center">
              ...
              <div className="flex items-center gap-6 justify-center">
                <div className="flex flex-col items-center gap-2">
                  <span className="text-xs text-slate-400 font-semibold">{match.teamHome}</span>
                  <input
                    type="text"
                    maxLength={2}
                    value={homeScore}
                ...
                <span className="text-slate-600 font-black text-3xl mt-6">-</span>

                <div className="flex flex-col items-center gap-2">
                  <span className="text-xs text-slate-400 font-semibold">{match.teamAway}</span>
                  <input
                    type="text"
                    maxLength={2}
                    value={awayScore}
              ...
              {!isLocked ? (
                ...
                >
                  {isSaving ? (
                    ...
                    <span>{prediction ? 'Actualizar Pronóstico' : 'Guardar Pronóstico'}</span>
                  )}
                </button>
              ) : (
                <div className="text-center p-3 text-xs text-slate-500 bg-slate-950/40 border border-slate-900 rounded-xl">
                  Formulario bloqueado por inicio de partido o cierre de ventana.
                </div>
              )}
        ...
        {match.status === 'FINISHED' && (
          <div className="bg-slate-900/60 border border-slate-850 p-6 rounded-3xl text-center space-y-3">
            <h4 className="text-xs uppercase font-bold text-slate-500 tracking-wider">Resultado del Pronóstico</h4>
            {prediction ? (
              <div className="space-y-3">
                <p className="text-sm">
                  Apostaste a un resultado de <strong className="text-white">{prediction.predictHome} - {prediction.predictAway}</strong>.
                </p>
                {prediction.pointsEarned !== null ? (
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-indigo-950/40 border border-indigo-500/25">
                    <span className="text-2xl font-black text-amber-400">+{prediction.pointsEarned}</span>
                    <div className="text-left text-xs">
                      <span className="block font-bold text-slate-200">
                        {prediction.pointsEarned === 3 && 'Puntos por acierto EXACTO!'}
                        {prediction.pointsEarned === 1 && 'Punto por acierto PARCIAL (Ganador/Empate).'}
                        {prediction.pointsEarned === 0 && 'Sin puntos obtenidos.'}
                      </span>
                      <span className="text-[10px] text-slate-400">Sumado a tu puntuación global</span>
                    </div>
                  </div>
                ) : null}
              </div>
            ) : (
              <p className="text-sm text-slate-500 italic">No registraste ningún pronóstico para este encuentro.</p>
            )}
          </div>
        )}
        ...
        <div className="bg-slate-900/40 border border-slate-900 p-6 rounded-3xl space-y-5">
          <div>
            <h4 className="text-sm font-bold text-slate-200">Tendencias de la Comunidad</h4>
            <p className="text-xs text-slate-400">Distribución de los pronósticos de los otros participantes</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="w-full bg-slate-950 h-2.5 rounded-full overflow-hidden border border-slate-850">
                <div className="bg-gradient-to-r from-indigo-500 to-indigo-600 h-full rounded-full transition-all duration-1000" style={{ width: '65%' }} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="w-full bg-slate-950 h-2.5 rounded-full overflow-hidden border border-slate-850">
                <div className="bg-gradient-to-r from-amber-500 to-yellow-500 h-full rounded-full transition-all duration-1000" style={{ width: '20%' }} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="w-full bg-slate-950 h-2.5 rounded-full overflow-hidden border border-slate-850">
                <div className="bg-gradient-to-r from-sky-400 to-sky-500 h-full rounded-full transition-all duration-1000" style={{ width: '15%' }} />
              </div>
            </div>
