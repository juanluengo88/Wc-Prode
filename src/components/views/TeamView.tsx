...
export default function TeamView({ teamId }: TeamViewProps) {
    ...
    return (
      <div className="flex-1 bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-slate-400 font-medium text-sm">{t("team_loading")}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 bg-slate-950 min-h-screen flex items-center justify-center p-4">
        <div className="text-center py-16 px-6 bg-slate-900/30 rounded-3xl border border-slate-900 max-w-md w-full">
          ...
          <p className="text-slate-200 font-semibold mb-1">{t("team_errorTitle")}</p>
          <p className="text-slate-400 text-sm">{error || t("team_errorMsg")}</p>
        </div>
      </div>
    );
  }
  ...
  const translatePosition = (pos: string) => {
    switch (pos) {
      case "Goalkeeper": return t("team_posGK");
      case "Defence": return t("team_posDF");
      case "Midfield": return t("team_posMF");
      case "Offence": return t("team_posFW");
      default: return t("team_posOther");
    }
  };
            ...
            <div className="flex items-center gap-4 text-sm text-slate-400">
              <p>
                ...
                <span className="text-slate-200 font-medium">
                  {team.coach || t("team_noCoach")}
                </span>
              </p>
              <span className="text-slate-700 hidden sm:inline">•</span>
              <p className="hidden sm:block">
                {t("team_tournamentId")}{" "}
                <span className="font-mono text-xs text-amber-500">{team.id}</span>
              </p>
            </div>
          </div>
        </div>
                      ...
                      >
                        ...
                        <div className="text-right space-y-1">
                          <span className="inline-block text-[10px] font-bold px-2 py-0.5 rounded bg-slate-800 text-slate-400 group-hover:bg-slate-750 group-hover:text-slate-300 transition-colors">
                            {player.nationality === team.name ? t("team_native") : player.nationality}
                          </span>
