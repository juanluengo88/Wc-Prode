...
export default function ProfileView({ user, onUpdateProfile, onLogout }: ProfileViewProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setErrorMessage('La imagen es demasiado grande. El límite recomendado es de 2MB.');
      return;
    }
      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-slate-900/80 border-b border-slate-880 px-4 py-4 sm:px-8">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">Edita tus datos de participante</p>
          </div>
          <button
            onClick={onLogout}
      ...
      <main className="max-w-xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="relative overflow-hidden p-6 rounded-3xl bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-slate-850 flex items-center justify-between gap-4">
          ...
          <div className="space-y-1">
            <span className="text-[10px] font-black text-amber-400 uppercase tracking-widest block">Resumen Competitivo</span>
          ...
          <div className="flex gap-4 shrink-0 bg-slate-950/60 p-3 rounded-2xl border border-slate-800 shadow-inner">
            <div className="text-center px-3 border-r border-slate-800">
              <span className="block text-xl font-black text-amber-400">{user.totalPoints}</span>
              <span className="text-[9px] text-slate-500 uppercase tracking-wider font-bold">Puntos</span>
            </div>
            <div className="text-center px-3">
              <span className="block text-xl font-black text-slate-350">#{user.rank ?? '-'}</span>
              <span className="text-[9px] text-slate-500 uppercase tracking-wider font-bold">Puesto</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-900 p-6 sm:p-8 rounded-3xl space-y-8">
          ...
          <div className="flex flex-col items-center gap-5">
            ...
            <div className="relative group">
              ...
              >
                ...
                <div className="absolute inset-0 bg-slate-950/65 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-1 transition-opacity duration-200">
                  ...
                  <span className="text-[9px] uppercase font-black text-slate-200 tracking-wider">Subir Foto</span>
                </div>
              </div>
            </div>
            ...
            <div className="text-center">
              <span className="text-xs font-bold text-slate-350 block">Foto de Perfil</span>
