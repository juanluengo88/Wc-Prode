...
export default function MatchCard({
  match, pred, locked, homeVal, awayVal,
  savingId, successId, isFormComplete,
  onSelectMatch, onInputChange, onSave, getPointsBadgeColor,
}: MatchCardProps) {
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4 py-2">
        <div className="flex-1 flex flex-col items-center text-center gap-2">
          <img src={match.teamHomeFlag} alt={match.teamHome} className="w-12 h-8 object-cover rounded-md shadow-md border border-slate-800" onError={(e) => { (e.target as HTMLElement).style.display = "none"; }} />
          <span className="text-xs sm:text-sm font-semibold truncate max-w-[100px] sm:max-w-none">{match.teamHome}</span>
        </div>

        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
          {match.status === "FINISHED" || match.status === "LIVE" ? (
            <div className="flex items-center gap-3">
              <span className="text-xl font-extrabold px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800">{match.scoreHome}</span>
              <span className="text-slate-500 font-bold text-sm">:</span>
              <span className="text-xl font-extrabold px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800">{match.scoreAway}</span>
            </div>
          ) : (
            <div className="flex items-center gap-1.5">
              <input type="text" maxLength={2} value={homeVal} disabled={locked} placeholder="-"
              ...
              <input type="text" maxLength={2} value={awayVal} disabled={locked} placeholder="-"
        ...
        <div className="flex-1 flex flex-col items-center text-center gap-2">
          <img src={match.teamAwayFlag} alt={match.teamAway} className="w-12 h-8 object-cover rounded-md shadow-md border border-slate-800" onError={(e) => { (e.target as HTMLElement).style.display = "none"; }} />
          <span className="text-xs sm:text-sm font-semibold truncate max-w-[100px] sm:max-w-none">{match.teamAway}</span>
        </div>
      </div>

      {/* Footer */}
      <div className="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
        <span className="font-semibold text-slate-500">{formattedDate} - {formattedTime} hs</span>
          ...
          {match.status === "FINISHED" ? (
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                successId === match.matchId ? "bg-emerald-500 text-slate-950"
                : isFormComplete ? "bg-amber-500 text-slate-950 hover:bg-amber-400 hover:scale-[1.03]"
